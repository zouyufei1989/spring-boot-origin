...
/**
 * Tests for {@link ConfigurationPropertiesReflectionHintsContribution}.
 *
 * @author Stephane Nicoll
 */
class ConfigurationPropertiesReflectionHintsContributionTests {
	...
	void processValueObjectConfigurationPropertiesWithSeveralLayersOfPojo() {
		...
		assertThat(typeHints).anySatisfy(valueObjectBinding(Person.class, Person.class.getConstructors()[0]));
	...
	private RuntimeHints process(Class<?>... types) {
		...
		new ConfigurationPropertiesReflectionHintsContribution(Arrays.asList(types)).applyTo(initialization);
		return generationContext.runtimeHints();
	}
