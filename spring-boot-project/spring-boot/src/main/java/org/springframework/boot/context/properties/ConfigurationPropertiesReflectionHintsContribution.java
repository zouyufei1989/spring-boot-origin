...
/**
 * A {@link BeanFactoryContribution} that provides reflection hints for relevant methods
 * of {@link ConfigurationProperties @ConfigurationProperties}-annotated types, their
 * nested types and any complex type that is bindable.
 *
 * @author Stephane Nicoll
 * @author Christoph Strobl
 * @author Sebastien Deleuze
 */
class ConfigurationPropertiesReflectionHintsContribution implements BeanFactoryContribution {

	private final Iterable<Class<?>> types;

	ConfigurationPropertiesReflectionHintsContribution(Iterable<Class<?>> types) {
		this.types = types;
	}
	...
	public void applyTo(BeanFactoryInitialization initialization) {
		...
		for (Class<?> type : this.types) {
			TypeProcessor.processConfigurationProperties(type, runtimeHints);
		}
	}
	...
	private static class TypeProcessor {
		...
		private boolean isClassOnlyReflectionType() {
			return this.type.getPackageName().startsWith("java.") || this.type.isArray();
		}
